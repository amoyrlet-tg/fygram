...
-- Lyrics are shared by tracks with the same normalised title and duration.  A row
-- with found = 0 is a cached miss, so opening the player does not repeatedly
-- query LRCLIB for songs that have no timed lyrics.
