...
pub(crate) fn setup(app: &mut tauri::App) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(window) = app.get_webview_window("main") {
        ...
        // Let DWM clip the custom frame and draw rounded corners on Windows 11.
        // Older Windows versions safely ignore this unsupported attribute.
        // SAFETY: hwnd belongs to our live window; preference is valid for the call.
        let _ = unsafe {
    ...
    crate::features::library::cache::sweep::spawn(app.handle().clone());
