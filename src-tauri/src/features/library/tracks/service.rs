...
pub(super) async fn download(
    ...
    let track = repository::get(&state.db, &track_id)
        .await?
        .ok_or_else(|| AppError::Msg("track not found".to_string()))?;
...
pub(super) async fn download_many(
    state: State<'_, AppState>,
    app: AppHandle,
    track_ids: Vec<String>,
    progress_id: String,
) -> Result<media::download::DownloadStats, AppError> {
    let ids: HashSet<_> = track_ids.into_iter().collect();
    if ids.is_empty() {
        return Ok(media::download::DownloadStats {
            downloaded: 0,
            failed: 0,
        });
    }

    let tracks = repository::list_ordered(&state.db)
        .await?
        .into_iter()
        .filter(|track| ids.contains(&track.id) && track.file_path.is_empty())
        .collect();
    let media_dir = media_paths::media_root(&app, &state.db).await?;
    Ok(media::download::download_tracks(
        &state.db,
        &state.telegram,
        &media_dir,
        tracks,
