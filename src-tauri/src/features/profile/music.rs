...
pub(crate) async fn list(state: State<'_, AppState>) -> Result<Vec<ProfileTrack>, AppError> {
    ...
    for entry in saved {
        let known = repository::track_for_document(&state.db, entry.document_id).await?;
...
pub(crate) async fn add(
    ...
    let input = library_document(&state, &track_id).await?;
...
async fn library_document(
    ...
    let track = repository::track(&state.db, track_id).await?;
    super::service::track_document(state, &track.channel_id, track.tg_message_id as i32).await
}
