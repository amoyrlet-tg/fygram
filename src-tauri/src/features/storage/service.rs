...
pub(crate) async fn info(
    ...
    let downloads =
        media_paths::downloads_media_root(&app).map_err(|err| AppError::Msg(err.to_string()))?;
...
pub(crate) async fn set_downloads_root(
    state: State<'_, AppState>,
    app: AppHandle,
) -> Result<RelocateResult, AppError> {
    let target =
        media_paths::downloads_media_root(&app).map_err(|err| AppError::Msg(err.to_string()))?;
    set_root_to_path(state, app, target, true).await
}
...
pub(crate) async fn set_root(
    ...
    set_root_to_path(state, app, target, move_existing).await
}

async fn set_root_to_path(
    state: State<'_, AppState>,
    app: AppHandle,
    target: PathBuf,
    move_existing: bool,
...
pub(crate) async fn ensure_layout(app: &AppHandle, db: &sqlx::SqlitePool) {
    let Ok(target) = media_paths::downloads_media_root(app) else {
        return;
    };
    let Ok(current) = media_paths::media_root(app, db).await else {
        return;
    };
    if current != target {
        if let Err(err) = tokio::fs::create_dir_all(&target).await {
            crate::log!("storage: could not create Downloads folder: {err}");
            return;
        }
        if let Err(err) = ensure_writable(&target).await {
            crate::log!("storage: Downloads folder is not writable: {err}");
            return;
        }
        match relocate(db, app, &target).await {
            Ok(result) => {
                crate::log!("storage: moved {} file(s) into Downloads", result.moved);
                if let Err(err) =
                    settings::set(db, media_paths::MEDIA_ROOT_KEY, &target.to_string_lossy()).await
                {
                    crate::log!("storage: could not save Downloads folder: {err}");
                    return;
                }
                media_paths::prune_empty_dirs(&current).await;
            ...
            Err(err) => {
                crate::log!("storage: could not move media into Downloads: {err}");
                return;
            }
        }
    }
    ...
    let Ok(root) = media_paths::downloads_media_root(app) else {
        return;
    };
