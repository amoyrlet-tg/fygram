...
import { toggleProfileMusic } from "@/features/profile/toggleProfileMusic";
import "./TrackRow.css";
import {
  EditIcon,
  GripIcon,
...
const EDIT_ACTIONS = {
  locked: { label: "No rights to edit in this channel", Icon: LockIcon },
  forwarded: { label: "Forwarded: editing means replacing the message", Icon: ForwardIcon },
  rename: { label: "Rename", Icon: EditIcon },
};

function editAction(canEdit: boolean | null | undefined, forwarded: boolean | null) {
  if (canEdit === false) return EDIT_ACTIONS.locked;
  if (forwarded) return EDIT_ACTIONS.forwarded;
  return EDIT_ACTIONS.rename;
}
          ...
          <>
            ...
            >
              <button
                ...
                onClick={() => {
                  closeMenu();
                  void toggleProfileMusic(trackId, t, "profile-music");
                }}
              >
...
function TrackRowCells({
  ...
  const isEditLocked = canEdit === false;
  const { label: editLabel, Icon: EditActionIcon } = editAction(canEdit, track.forwarded);
  ...
  return (
    <>
      ...
      <td className="col-index">
        ...
        >
          ...
          <span className="row-play-icon">
            {playingHere ? <PauseIcon size={14} /> : <PlayIcon size={14} />}
          </span>
        </button>
      </td>
      <td>
      ...
      <td className="col-actions">
        <button
          className={`icon-btn${isEditLocked ? " is-locked" : ""}`}
          title={t(editLabel)}
          onClick={startEdit}
        >
          <EditActionIcon size={14} />
        </button>
        {onRemoveFromPlaylist ? (
          <button
            className="icon-btn"
