...
import type { Channel, DownloadProgress, Playlist, Track } from "@/shared/api/types";
import { formatDownloadSpeed, formatRuntime, totalDurationSeconds } from "@/shared/lib/format";
...
import { HeaderActions, type HeaderSubject } from "./HeaderActions";
...
const OVERSCAN = 10;
const SCROLL_BUCKET = ROW_HEIGHT * 4;

const NEW_WINDOW_MS = 3 * 24 * 60 * 60 * 1000;

function ChannelFace({ path, seed, title }: { path?: string | null; seed: string; title: string }) {
  const fallback = (
    <span style={{ background: avatarGradientCss(seed) }}>{title.slice(0, 1).toUpperCase()}</span>
  );
  if (!path) return fallback;
  return <UserAvatar className="" path={path} fallback={fallback} />;
}
...
export const TrackTable = memo(function TrackTable({
  ...
  onRenameArtist?: (artistName: string, newName: string) => Promise<void>;
  ...
  onUpdateTrack: (trackId: string, edit: TrackUpdate) => Promise<boolean>;
  onDownloadTracks?: (trackIds: string[]) => Promise<void>;
  ...
  const [downloadingTracks, setDownloadingTracks] = useState(false);
  ...
  const handleScroll = useCallback(() => {
    ...
    scrollRafRef.current = requestAnimationFrame(() => {
      ...
      const pinAt = Math.max(0, top - PIN_HEIGHT);
      setScrollTop((prev) => {
        const sameBucket =
          Math.floor(Math.max(0, prev - top) / SCROLL_BUCKET) ===
          Math.floor(Math.max(0, next - top) / SCROLL_BUCKET);
        return sameBucket && prev >= pinAt === next >= pinAt ? prev : next;
      });
    });
  }, []);
  ...
  const naming = (():
    { label: string; current: string; commit: (next: string) => void } | undefined => {
    if (channelView) {
      return channelView.can_edit
        ? {
            label: t("Rename channel"),
            current: channelView.title,
            commit: (next) => onRenameChannel?.(channelView.id, next),
          }
        : undefined;
    }
    if (playlistView) {
      return {
        ...
        current: playlistView.name,
        commit: (next) => onRenamePlaylist?.(playlistView.id, next),
      };
    }
    if (artistName) {
      return {
        label: t("Rename artist"),
        current: artistName,
        commit: (next) => void onRenameArtist?.(artistName, next),
      };
    }
    return undefined;
  })();
  const submitRename = () => {
    ...
    if (next && naming && next !== naming.current) naming.commit(next);
    setRenameDraft(null);
  };
  const isEmpty = total === 0;
  const mosaicIds = useMemo(() => tracks.slice(0, 32).map((tr) => tr.id), [tracks]);
  ...
  useLayoutEffect(() => {
    ...
    const update = () => {
      ...
      if (wrap) {
        tableTopRef.current = wrap.offsetTop;
        setTableTop(wrap.offsetTop);
      }
    };
    update();
  ...
  const isDownloadPending = !!downloadProgress && !downloadProgress.done;
  const downloadSpeed = isDownloadPending
    ? formatDownloadSpeed(downloadProgress.bytes_per_second)
    : "";
  const columnCount = 4 + (compact ? 0 : 2) + (reorderable ? 1 : 0);
  ...
  const downloadArtistTracks = useCallback(async () => {
    if (!onDownloadTracks || downloadingTracks) return;
    setDownloadingTracks(true);
    try {
      await onDownloadTracks(tracks.map((track) => track.id));
    } catch (err) {
      alert(`${t("Couldn't download the track:")}\n${err}`);
    } finally {
      setDownloadingTracks(false);
    }
  }, [downloadingTracks, onDownloadTracks, t, tracks]);

  const renameAction = naming
    ? { label: naming.label, run: () => setRenameDraft(naming.current) }
    : undefined;
    ...
    if (channelView) {
      return {
        sync: {
          busy: !!channelBusy,
          run: () =>
            channelBusy ? onCancelSync?.(channelView.id) : onSyncChannel?.(channelView.id),
        },
        download: () => onDownloadChannel?.(channelView.id),
        rename: renameAction,
        remove: { label: t("Delete channel"), run: () => onDeleteChannel?.(channelView.id) },
      };
    }
    if (playlistView) {
      return {
        download: () => onDownloadPlaylist?.(playlistView.id),
        rename: renameAction,
        remove: { label: t("Delete playlist"), run: () => onDeletePlaylist?.(playlistView.id) },
      };
    }
    if (artistName) {
      return {
        download: artistView && onDownloadTracks ? () => void downloadArtistTracks() : undefined,
        rename: renameAction,
      };
    }
    return {};
  ...
  const channelFace = channelView ? (
    <ChannelFace path={channelView.avatar_path} seed={channelView.id} title={title} />
  ) : null;
  const channelArt =
    ...
    <section className="track-view" ref={scrollRef} onScroll={handleScroll}>
      {hasHero && (
        <header className="channel-hero" data-tauri-drag-region>
          <div className={`channel-hero-art${channelView ? " is-channel" : ""}`}>
            {playlistView ? (
              <>
                <button
                  type="button"
      ...
      {hasHero && (
        <div className="channel-hero-bar">
          <HeaderActions
            key={`${channelView?.id ?? playlistView?.id ?? artistName ?? "library"}`}
            playing={playingHere}
            disabled={total === 0}
            downloading={downloadingTracks || isDownloadPending}
            onPlay={playAll}
            onShuffle={() => onPlay(tracks, Math.floor(Math.random() * tracks.length))}
            subject={subject}
          />
          {isDownloadPending && (
            <div className="channel-hero-download-state">
              <div
                className="channel-hero-download-progress"
                role="progressbar"
                aria-label={t("Downloading…")}
                aria-valuemin={0}
                aria-valuemax={downloadProgress.total || undefined}
                aria-valuenow={downloadProgress.total ? downloadProgress.processed : undefined}
              >
                <span
                  style={{
                    width: downloadProgress.total
                      ? `${Math.min(100, (downloadProgress.processed / downloadProgress.total) * 100)}%`
                      : "8%",
                  }}
                />
              </div>
              <div className="channel-hero-download-meta">
                <span>
          ...
          >
            <span className="track-view-pin-art">
              {playlistView ? (
                ...
                <ChannelFace path={channelView?.avatar_path} seed={title} title={title} />
              )}
            </span>
