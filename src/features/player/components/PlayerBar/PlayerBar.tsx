...
export function PlayerBar({
  ...
  const palette = cover?.palette?.length
    ? cover.palette
    : current
      ? avatarPaletteCss(current.id)
      : [];
  const tint = palette[0] ?? "transparent";
  ...
  return (
    <footer
      className={`player-bar${nowPlayingOpen ? " is-over-now-playing" : ""}`}
      style={{ "--player-tint": tint } as CSSProperties}
    >
