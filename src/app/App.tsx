...
function App() {
  ...
  useEffect(() => {
    const unlisten = listen("logged-out", () => {
      localStorage.removeItem("profileAccentColor");
  ...
  useEffect(() => {
    ...
    const id = requestAnimationFrame(() => setLibraryMounted(true));
  ...
  if (screen === "loading") {
    content = (
      <div className="boot-screen" role="status" aria-busy="true">
        <div className="boot-brand">fygram</div>
        <div className="boot-progress" aria-hidden="true" />
        <div className="boot-caption">{t("Loading fygram…")}</div>
      </div>
    );
